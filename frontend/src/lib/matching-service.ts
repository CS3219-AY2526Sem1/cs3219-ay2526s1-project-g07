...
export const matchingService = {
  ...
  async startMatching(request: UserMatchingRequest): Promise<void> {
    const matchingBaseUrl = import.meta.env.PUBLIC_MATCHING_SERVICE_URL || 'http://localhost:3000';
    try {
      const response = await fetch(`${matchingBaseUrl}${API_ENDPOINTS_MATCHING.MATCHING_REQUEST}`, {
        method: 'POST',
        headers: {
  ...
  async cancelMatching(request: UserMatchingCancelRequest): Promise<void> {
    const matchingBaseUrl = import.meta.env.PUBLIC_MATCHING_SERVICE_URL || 'http://localhost:3000';
    try {
      const response = await fetch(`${matchingBaseUrl}${API_ENDPOINTS_MATCHING.MATCHING_CANCEL}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
